'use client';

import React from 'react';
import Link from 'next/link';
import { ShieldCheck, AlertCircle, ArrowRight, Lock } from 'lucide-react';
import { useTeam } from '@/context/TeamContext';

export function CompletionGateBanner() {
  const { state } = useTeam();

  const completedCount = state.members.filter((m) => m.status === 'complete').length;
  const isFullyAssembled = completedCount >= 3 && completedCount === state.members.length;

  return (
    <div
      className={`p-6 rounded-card border shadow-sm transition-all ${
        isFullyAssembled
          ? 'bg-emerald-50/90 border-emerald-200 text-emerald-950'
          : 'bg-clay/10 border-clay/30 text-clay'
      }`}
      data-layer="SOURCE"
    >
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-start gap-3.5">
          {isFullyAssembled ? (
            <ShieldCheck className="w-5 h-5 text-emerald-700 shrink-0 mt-0.5" />
          ) : (
            <Lock className="w-5 h-5 text-clay shrink-0 mt-0.5" />
          )}

          <div className="space-y-1">
            <h4 className="font-heading font-bold text-sm">
              {isFullyAssembled
                ? `100% Profile Assembly Complete (${completedCount} of ${state.members.length} Connected)`
                : `Ecclesial Completion Gate Active (${completedCount} of ${state.members.length} Ready)`}
            </h4>
            <p className="text-xs leading-relaxed max-w-2xl font-body opacity-90">
              {isFullyAssembled
                ? 'Every leader on the team has attached their verified 5Q vocational scores. You are ready to calibrate qualitative institutional context.'
                : 'APEST Teams evaluates the living ecology between members. A partial team picture produces a false diagnosis. All seats must be filled before synthesis activates.'}
            </p>
          </div>
        </div>

        <div>
          {isFullyAssembled ? (
            <Link
              href="/setup/context"
              className="inline-flex items-center gap-2 px-6 py-2.5 rounded-button bg-primary text-primary-foreground font-heading font-semibold text-xs hover:bg-primary-hover transition-all shadow-sm whitespace-nowrap"
            >
              <span>Calibrate Context</span>
              <ArrowRight className="w-4 h-4 text-clay" />
            </Link>
          ) : (
            <button
              disabled
              className="inline-flex items-center gap-1.5 px-5 py-2.5 rounded-button bg-surface-muted text-ink-tertiary font-heading font-semibold text-xs cursor-not-allowed opacity-70 whitespace-nowrap"
            >
              <span>Gate Locked</span>
            </button>
          )}
        </div>
      </div>
    </div>
  );
}
