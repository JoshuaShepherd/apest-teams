...
const FUNCTION_COLORS: Record<ApestFunction, { bg: string; text: string; bar: string }> = {
  ...
  prophet: { bg: 'bg-indigo-50', text: 'text-indigo-800', bar: 'bg-indigo-700' },
  evangelist: { bg: 'bg-amber-50', text: 'text-amber-800', bar: 'bg-amber-600' },
...
export function MemberProfileDrawer() {
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end bg-black/40 backdrop-blur-sm animate-fade-in no-print">
      <div
        className="w-full max-w-lg bg-surface-card h-full shadow-2xl flex flex-col border-l border-surface-border overflow-hidden animate-slide-left"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Header */}
        <div className="p-6 border-b border-surface-border flex items-start justify-between gap-4 bg-surface-subtle">
          <div className="flex items-center gap-3">
            <div className="w-12 h-12 rounded-full bg-ink-primary text-white flex items-center justify-center font-bold text-lg font-serif">
              {activeMember.name
                .split(' ')
                .map((n) => n[0])
                .join('')}
            ...
            <div>
              <div className="flex items-center gap-2">
                <h3 className="text-lg font-bold text-ink-primary">{activeMember.name}</h3>
                {activeMember.hasFormalAuthority && (
                  <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded text-[10px] font-semibold uppercase tracking-wider bg-rose-100 text-rose-800 border border-rose-200">
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          ...
          <div className="p-4 rounded-xl bg-surface-subtle border border-surface-border space-y-2">
            <div className="flex items-center justify-between">
              <span className="text-xs uppercase font-mono text-ink-tertiary">Vocational Combination</span>
          ...
          <div className="space-y-3">
            ...
            <div className="space-y-2.5">
              {(['apostle', 'prophet', 'evangelist', 'shepherd', 'teacher'] as const).map((func) => {
                ...
                return (
                  <div key={func} className="p-3 rounded-lg border border-surface-border bg-white space-y-1.5">
                    <div className="flex items-center justify-between text-xs">
                      <div className="flex items-center gap-2">
                        <span className="font-semibold capitalize text-ink-primary">{func}</span>
                        {isPrimary && (
                          <span className="px-1.5 py-0.5 rounded text-[10px] font-bold uppercase bg-rose-50 text-rose-800">
                            Primary
                          </span>
                        )}
                        {isSecondary && (
                          <span className="px-1.5 py-0.5 rounded text-[10px] font-bold uppercase bg-indigo-50 text-indigo-800">
                            Secondary
                          </span>
                        )}
                      </div>
                      <div className="flex items-center gap-2 font-mono">
                        <span className="font-bold text-ink-primary">{scoreData.score}/50</span>
                        {benchmark && (
            ...
            <div className="space-y-3">
              ...
              <div className="space-y-2">
                {activeMember.profile.supplementaryMaturity.map((step) => (
                  <div key={step.key} className="p-3 rounded-lg bg-surface-subtle border border-surface-border text-xs space-y-1">
                    <div className="flex items-center gap-1.5 font-semibold capitalize text-ink-primary">
