  ...
  return (
    <div className="p-6 sm:p-8 rounded-2xl bg-white border border-surface-border shadow-sm space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
        <div>
          <h3 className="font-serif font-bold text-lg text-ink-primary">
            Unified Team Ecclesial Health Score
          </h3>
        ...
        <div className="flex items-center gap-3 p-4 rounded-xl bg-indigo-50 border border-indigo-200">
          <div className="text-center">
            <span className="text-[10px] uppercase font-mono text-indigo-700 font-semibold">
              Composite Index
            </span>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 text-xs">
        {BREAKDOWNS.map((b, i) => (
          <div key={i} className="p-4 rounded-xl bg-surface-subtle border border-surface-border space-y-2">
            <div className="flex items-center justify-between">
              <span className="font-bold text-ink-primary">{b.label}</span>
              <span className="font-mono text-[10px] text-ink-tertiary">{b.weight}</span>
            </div>

            <div className="flex items-center justify-between">
              <span className="text-xl font-bold font-mono text-ink-primary">{b.score}%</span>
